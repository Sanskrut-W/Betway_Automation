...
export class HomePage extends BasePage {
    ...
    constructor(page: import('@playwright/test').Page) {
        ...
        this.HomePagelocatorsRegistry = {
            ...
            BettingRules: this.page.locator('a[href="/betting-rules"]').last(),
            ...
            AboutUs: this.page.locator('a[href="/about-us"]').last(),
    ...
    async verifyAboutUsLink() {
        await this.HomePagelocatorsRegistry.AboutUs.waitFor({ state: 'visible', timeout: 10000 });
        await highlightElements(this.HomePagelocatorsRegistry.AboutUs);
    }
    ...
    async clickFAQSBackButton() {
        await highlightElementBorder(this.page.getByRole('heading', { name: "Frequently Asked Questions" }).first().locator('..').locator('a').first());
        await this.page.getByRole('heading', { name: "Frequently Asked Questions" }).first().locator('..').locator('a').first().click();
        await this.page.waitForLoadState('domcontentloaded');
        await expect(this.page).toHaveURL('/', { timeout: 10000 });
    }
    ...
    async clickBettingRulesBackButton() {
        await highlightElementBorder(this.page.getByRole('heading', { name: "Betting Rules" }).first().locator('..').locator('a').first());
        await this.page.getByRole('heading', { name: "Betting Rules" }).first().locator('..').locator('a').first().click();
        await this.page.waitForLoadState('domcontentloaded');
        await expect(this.page).toHaveURL('/', { timeout: 10000 });
    }
    ...
    async clickAboutUsLink() {
        await this.HomePagelocatorsRegistry.AboutUs.click({ force: true });
        await expect(this.page).toHaveURL(/.*about-us*/i, { timeout: 15000 });
        await this.page.waitForLoadState('domcontentloaded');
    }

    async clickPlayAviatorLink() {
        await this.HomePagelocatorsRegistry.PlayAviator.click({ force: true });
        await expect(this.page).toHaveURL(/.*aviator*/i, { timeout: 15000 });
        await this.page.waitForLoadState('domcontentloaded');
    }
