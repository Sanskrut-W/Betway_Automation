...
/**
 * Shared base for every GH page object.
...
export class BasePage {
    ...
    /**
     * The standard header-login flow. Exposed separately so subclasses whose
     * ancestor chain contains a *different* Login() variant (e.g. the betslip
     * branch, where LoginPage defines a home-page/verify login) can reassert
     * the standard behavior without re-implementing it.
     */
    protected async baseLogin(user: { mobile: string; password: string } = userData.user4) {
