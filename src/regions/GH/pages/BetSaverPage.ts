...
export class BetSaverPage extends BasePage {
    ...
    // This region uses the explicit Login button (not Enter) and no popup wait,
    // so it keeps its own Login() instead of the BasePage flow.
    async Login() {
    ...
    async outrightsSelection() {
        await this.page.goto('/sport/soccer/football/outrights');
        await this.page.waitForTimeout(2000);
