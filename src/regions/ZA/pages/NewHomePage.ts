...
/**
 * NewHomePage  —  Excel sheet: "New Home page"
 * Covers the redesigned Betway ZA homepage: header, navigation bar and hero banner,
 * for both the logged-out (TC_BL_*) and logged-in (TC_HP_*) states.
 *
 * Extends HeaderPage so we reuse its login + header helpers. All selectors below were
 * CONFIRMED against the live https://www.betway.co.za DOM (see // Locator confirmed notes),
 * mirroring the approach used for the Hamburger Menu feature.
 */
export class NewHomePage extends HeaderPage {
    ...
    // ── Navigation / setup ───────────────────────────────────────────────────
    async gotoHome() {
        await this.page.goto('https://www.betway.co.za/', { waitUntil: 'domcontentloaded' });
    ...
    async verifyTitleAndUrl() {
        await expect(this.page).toHaveURL('https://www.betway.co.za/');
        await expect(this.page).toHaveTitle(/Betway South Africa/i);
    }
