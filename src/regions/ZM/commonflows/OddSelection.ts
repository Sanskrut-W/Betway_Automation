...
export async function OddsSelection(numberOflegs: number, page: import('@playwright/test').Page) {
    ...
    const apiUrl = "https://www.betway.co.zm/sportsapi/br/v1/BetBook/Upcoming/?countryCode=ZM&sportId=soccer";
...
export async function EsportsOddsSelection(numberOflegs: number, page: import('@playwright/test').Page) {
    await page.reload({ waitUntil: 'domcontentloaded' });
    const apiUrl = "https://www.betway.co.zm/sportsapi/br/v1/BetBook/Upcoming/?countryCode=ZM&sportId=esports-league-of-legends&Skip=0&Take=20&cultureCode=en-US&isEsport=true&boostedOnly=false&marketTypes=%5BMatch%20Winner%5D";
    const response = await page.waitForResponse((resp: { url: () => string; status: () => number; }) => resp.url().startsWith(apiUrl) && resp.status() === 200);
    const data = await response.json();
...
export async function LiveOddsSelection(numberOfLegs: number, page: import('@playwright/test').Page) {
    ...
    const apiUrl = "https://www.betway.co.zm/sportsapi/br/v1/BetBook/Upcoming/?countryCode=ZM&sportId=table-tennis"
    const response = await page.waitForResponse((resp: { url: () => string; status: () => number; }) => resp.url().startsWith(apiUrl) && resp.status() === 200);
    const data = await response.json();
    for (let i = 0; i < numberOfLegs; i++) {
        const eventId = data.events?.[i]?.eventId;
...
//     const apiUrl = "https://betway.co.zm/sportsapi/br/v1/BetBook/Upcoming/?countryCode=ZM&sportId=soccer";
