...
test.describe('Footer Module Tests', () => {
    ...
    test('T32 b -Verify functionality of "Betting Rules" at footer section', async ({ homePage }, testInfo) => {
        ...
        await expect(newPage).toHaveURL('https://cms1.gmgamingsystems.com/medialibraries/content.gmgamingsystems.com/Betting-Rules/BettingRules_MW_en.pdf');
    ...
    test('T68-Verify functionality of "MAGLA Contact" button at the Footer section', async ({ homePage }, testInfo) => {
        await homePage.clickMAGLAContactLink();
        await ScreenshotHelper(homePage.page, screenshotDir, 'T68.png', testInfo);
    });

});
// npx playwright test src/regions/MW/tests/smoke/footer/footer.spec.ts --config=playwright.MW.config.ts --headed
// 2.allure generate allure-results --clean -o allure-report
// 3.allure open src/regions/MW/reports/allure-report 