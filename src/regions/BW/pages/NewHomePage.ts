 ...
 * Covers the redesigned Betway BW homepage: header, navigation bar and hero banner,
 ...
 * independently verified against BW's own site.
 */
export class NewHomePage extends HeaderPage {
    ...
    // ── Navigation / setup ───────────────────────────────────────────────────
    async gotoHome() {
        await this.page.goto('https://www.betway.co.bw/', { waitUntil: 'domcontentloaded' });
    ...
    async verifyTitleAndUrl() {
        await expect(this.page).toHaveURL('https://www.betway.co.bw/');
